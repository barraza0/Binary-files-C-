...
int ingresa(struct lid **);
int lee(struct lid **);
int borra(struct lid **);
int busca(struct lid **);
int endat(struct lid **);
...
struct binario Dat; struct lid *pr=NULL;
    ...
    for(;;){
        while(op!=6){
            printf("1__agregar registro \n2__eliminar registro \n3__listar todos los registros \n4__listar conjunto seleccionado \n5__salir de la aplicacion \n:");
            fflush(stdin);
            scanf("%d",&op);
            switch(op){
            case 1: ingresa(&pr);break;
            case 2: borra(&pr);break;
            case 3: lee(&pr);break;
            case 4: busca(&pr);break;
            case 5: exit(0);
            }
        }
    }
}
int ingresa(struct lid **p){
